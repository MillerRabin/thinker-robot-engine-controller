...
void ArmElbow::engineTask(void *instance) {
  ...
  while (true) {
    ...
    vTaskDelayUntil(&lastWakeTime, elbow->taskInterval);
  }
}
...
ArmElbow::ArmElbow(const uint memsSdaPin, const uint memsSclPin,
                   ...
                   const uint canTxPin)
    : ArmPart(canRxPin, canTxPin), elbowY(engineYPin, Range(0, 270), NAN, 100),
      imu(this, memsSdaPin, memsSclPin, memsIntPin, memsRstPin) {}

int ArmElbow::begin() {
  if (xTaskCreate(ArmElbow::engineTask, "ArmElbow::engineTask", 4096, this, 5,
                  NULL) == pdFAIL) {
