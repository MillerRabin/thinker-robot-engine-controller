...
# Deploy Shoulder Control Script
# Uses pico-mount.sh to mount Pico and copies ENGINE_CONTROL_SHOULDER.uf2
# Runs without sudo using udisksctl via pico-mount.sh

# Get the directory where this script is located
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
UF2_FILE="$SCRIPT_DIR/build/shoulder-control/ENGINE_CONTROL_SHOULDER.uf2"
...
echo "🦾 Shoulder Control Deployment Script"
echo "==============================================="
...
if [ ! -f "$UF2_FILE" ]; then
    ...
    echo "   Build may have failed or not targeted shoulder-control."
    exit 1
fi
...
# Copy UF2 file
echo -e "${BLUE}🚀 Copying shoulder control firmware...${NC}"
...
if [ $? -eq 0 ]; then
    ...
    # Verify the copy
    if [ -f "$MOUNT_POINT/ENGINE_CONTROL_SHOULDER.uf2" ]; then
        echo -e "   Target file size: $(ls -lh "$MOUNT_POINT/ENGINE_CONTROL_SHOULDER.uf2" | awk '{print $5}')"
    fi
else
